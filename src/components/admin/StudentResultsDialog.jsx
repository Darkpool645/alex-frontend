                    ...
                    <div className="flex min-h-full items-center justify-center p-4 text-center">
                        <TransitionChild as={Fragment} enter="ease-out duration-300"
                            ...
                            <DialogPanel className="w-full max-w-6xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                <div className="mt-4">
                                    {loading ? (
                                        <div className="text-center text-gray-500">Cargando alumnos...</div>
                                    ) : (
                                        <table className="w-full table-auto">
                                            <thead>
                                                <tr>
                                                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-900">Alumno</th>
                                                    <th className="px-4 py-2 text-left text-sm font-semibold text-gray-900">Puntuación</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                {studentResults.length === 0 ? (
                                                    ...
                                                    studentResults.map((student, idx) => (
                                                        <tr key={idx}>
                                                            <td className="px-4 py-2 border-t">{student.studentName}</td>
                                                            <td className="px-4 py-2 border-t">{student.score}/{student.questions.length}</td>
                                                        </tr>
                                                    ))
                                                )}
                                            </tbody>
